...
(
    web_application        varchar(255)             null comment 'The name of the Web Application that owns this cookie storage',
    name                   varchar(255)             null comment 'The name of the cookie',
    token                  varchar(255)             not null comment 'The value of the cookie (Token)' primary key,
    ...
    constraint cookie_storage_name_value_uindex unique (name, token),
    constraint cookie_storage_web_application_name_token_uindex unique (web_application, name, token)
)
    comment 'Stores DynamicalWeb cookie storage objects';
...
create index cookie_storage_value_uindex
    on cookie_storage (token);

create index cookie_storage_web_application_index
    on cookie_storage (web_application);
